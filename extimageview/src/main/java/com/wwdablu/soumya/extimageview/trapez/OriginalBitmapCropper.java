...
class OriginalBitmapCropper implements Runnable {
    ...
    OriginalBitmapCropper(@NonNull Bitmap originalBitmap,
                          ...
                          @NonNull ResultCallBack<Bitmap> resultCallBack,
    ...
    public void run() {
        ...
        try {
            ...
            mResultCallBack.onComplete(croppedAndCorrected);
        } catch (Exception ex) {
    ...
    private Bitmap trapezoidToRectangleTransform() {
        ...
        if (mapped) {
            Log.e("DucLH", "trapezoidToRectangleTransform: " + mOriginalBitmap.getWidth() + "ss " + mOriginalBitmap.getHeight() );
